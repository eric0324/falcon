...
## Task 1: Slack Client
- [x] 建立 `src/lib/integrations/slack/client.ts`
...
- [x] 建立 `src/lib/integrations/slack/index.ts` re-export
- [x] 建立 `src/lib/integrations/slack/client.test.ts` 單元測試
- **驗證**: `npm test -- slack/client` 全綠

## Task 2: Slack AI Tool
- [x] 建立 `src/lib/ai/slack-tools.ts`
  - `createSlackTools()` 回傳 `{ slackSearch }` 工具
  - action: list / read / thread / search
  - 回傳輕量格式，避免 token 浪費
- [x] 建立 `src/lib/ai/slack-tools.test.ts` 單元測試
- **驗證**: `npm test -- slack-tools` 全綠

## Task 3: Route Integration
- [x] `src/app/api/chat/route.ts`: import slackTools，加入 dataSources 過濾邏輯
- [x] `src/app/api/integrations/status/route.ts`: 回報 `slack: isSlackConfigured()`
- **驗證**: 手動測試 `/api/integrations/status` 回傳 slack 狀態

## Task 4: System Prompt
- [x] `src/lib/ai/system-prompt.ts`: 加入 Slack 使用指南區塊
  - 工具操作說明
  - 搜尋策略（list → read/search → thread）
- **驗證**: 選擇 Slack 資料來源時，system prompt 包含指南

## Task 5: UI Integration
- [x] `src/components/data-source-selector.tsx`: 第三方服務加入 Slack 選項
- [x] 更新 i18n：`slack.description` 改為「搜尋和瀏覽 Slack 公開頻道」
- [x] `.env.example`: 加入 `SLACK_BOT_TOKEN` 和 `SLACK_USER_TOKEN`

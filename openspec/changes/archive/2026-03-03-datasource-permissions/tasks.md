...
- [x] 1.1 實作 SQL parser 判斷操作類型：
  ...
  ```
- [x] 1.2 處理 CTE (WITH ... SELECT) 的情況
- [x] 1.3 處理多語句的情況 (禁止或只取第一個)

## 2. 權限 Model 設計
- [x] 2.1 建立 DataSourcePermission model：
  ```prisma
...
- [x] 3.1 在 /api/bridge 加入權限檢查：
  ...
  ```
- [x] 3.2 OTHER 類型一律禁止 (DROP, ALTER, TRUNCATE 等)
- [x] 3.3 記錄被攔截的操作 (logging)
...
## 5. 預設權限邏輯
- [x] 5.1 無 DataSourcePermission 記錄 = 只有 SELECT
- [x] 5.2 Tool 首次使用資料來源時自動建立唯讀權限記錄 (可選)

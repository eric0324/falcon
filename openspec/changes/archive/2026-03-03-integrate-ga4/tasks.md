...
## Task 2: GA4 Client
- [x] 建立 `src/lib/integrations/ga4/client.ts`
...
- [x] 建立 `src/lib/integrations/ga4/index.ts` re-export
- [x] 建立 `src/lib/integrations/ga4/client.test.ts` 單元測試
- **驗證**: `npm test -- ga4/client` 全綠

## Task 3: GA4 AI Tool
- [x] 建立 `src/lib/ai/ga4-tools.ts`
...
- [x] 建立 `src/lib/ai/ga4-tools.test.ts` 單元測試
- **驗證**: `npm test -- ga4-tools` 全綠

## Task 4: Route Integration
- [x] `src/app/api/chat/route.ts`: import ga4Tools，加入 dataSources 過濾邏輯
- [x] `src/app/api/integrations/status/route.ts`: 回報 `ga4: isGA4Configured()`
- **驗證**: `/api/integrations/status` 回傳 ga4 狀態

## Task 5: System Prompt
- [x] `src/lib/ai/system-prompt.ts`: 加入 GA4 使用指南區塊
  - 可用 actions 說明
  - dimension 選項列表
  - dateRange 選項列表
  - filter 用法範例
...
- [x] `src/components/data-source-selector.tsx`:「網站數據」子選單加入 GA4 選項（BarChart icon）
- [x] `src/components/tool-call-display.tsx`: 加入 ga4Query 的圖示和標籤
- [x] 更新 i18n（en.json, zh-TW.json）：加入 ga4 name/description
- [x] `.env.example`: 加入 `GA4_CLIENT_EMAIL`, `GA4_PRIVATE_KEY`, `GA4_PROPERTY_ID`

...
AI SDK 的 `generateImage()` 只支援文生圖（沒有 input image 參數），且 Google Imagen 4 本身 API 不支援圖生圖。為保持「兩 provider 都能圖生圖」的 scope，各家分頭走原生 REST：

**文生圖**（走 AI SDK `generateImage`）：
- `imagen` → `google.image("imagen-4.0-generate-001")`
- `gpt-image` → `openai.image("gpt-image-1")`

**圖生圖**（繞開 AI SDK，直接 REST）：
- `imagen` provider 實際走 **Gemini 2.5 Flash Image**（Nano Banana），REST endpoint `POST .../v1beta/models/gemini-2.5-flash-image:generateContent`。原圖以 base64 inlineData 傳入。使用既有 `GOOGLE_GENERATIVE_AI_API_KEY`。
- `gpt-image` → `POST https://api.openai.com/v1/images/edits`，multipart/form-data 帶 `image` 檔案 + `prompt` + `model=gpt-image-1`。使用既有 `OPENAI_API_KEY`。

**理由**：
- AI SDK 短期內沒看到會加 input image 支援
- 保持對外 provider 名稱一致（`imagen` / `gpt-image`），使用者 UX 不變
- 內部實作兩條路徑是合理取捨

**Tradeoff**：
- `imagen` provider 在圖生圖時實際上是 Gemini Image，不是 Imagen。這是隱藏的實作細節，需在 code comment 說明，避免未來維護者誤解
- 兩條路徑代表兩套 error handling 與計費記錄要分別維護

### 7. S3 Credentials 透過 SystemConfig 管理

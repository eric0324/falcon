...
- [x] 建立 `src/lib/integrations/plausible/client.ts`
...
- [x] 建立 `src/lib/integrations/plausible/index.ts` re-export
- [x] 建立 `src/lib/integrations/plausible/client.test.ts` 單元測試
- **驗證**: `npm test -- plausible/client` 全綠

## Task 2: Plausible AI Tool
- [x] 建立 `src/lib/ai/plausible-tools.ts`
...
- [x] 建立 `src/lib/ai/plausible-tools.test.ts` 單元測試
- **驗證**: `npm test -- plausible-tools` 全綠

## Task 3: Route Integration
- [x] `src/app/api/chat/route.ts`: import plausibleTools，加入 dataSources 過濾邏輯
- [x] `src/app/api/integrations/status/route.ts`: 回報 `plausible: isPlausibleConfigured()`
- **驗證**: `/api/integrations/status` 回傳 plausible 狀態

## Task 4: System Prompt
- [x] `src/lib/ai/system-prompt.ts`: 加入 Plausible 使用指南區塊
  - 可用 actions 說明
  - dimension 選項列表
  - dateRange 選項列表
  - filter 用法範例
...
- [x] `src/components/data-source-selector.tsx`: 第三方服務加入 Plausible 選項（BarChart3 icon）
- [x] `src/components/tool-call-display.tsx`: 加入 plausibleQuery 的圖示和標籤
- [x] 更新 i18n（en.json, zh-TW.json）：加入 plausible name/description
- [x] `.env.example`: 加入 `PLAUSIBLE_API_KEY`, `PLAUSIBLE_SITE_ID`, `PLAUSIBLE_BASE_URL`

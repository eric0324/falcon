...
## Task 1: Asana Client
- [x] 建立 `src/lib/integrations/asana/client.ts`
  - `isAsanaConfigured()`
...
- [x] 建立 `src/lib/integrations/asana/index.ts` re-export
- [x] 建立 `src/lib/integrations/asana/client.test.ts` 單元測試
- **驗證**: `npm test -- asana/client` 全綠

## Task 2: Asana AI Tool
- [x] 建立 `src/lib/ai/asana-tools.ts`
  - `createAsanaTools()` 回傳 `{ asanaSearch }` 工具
  - action: list / tasks / read / comments / search
  - 回傳輕量格式
- [x] 建立 `src/lib/ai/asana-tools.test.ts` 單元測試
- **驗證**: `npm test -- asana-tools` 全綠

## Task 3: Route Integration
- [x] `src/app/api/chat/route.ts`: import asanaTools，加入 dataSources 過濾邏輯
- [x] `src/app/api/integrations/status/route.ts`: 回報 `asana: isAsanaConfigured()`
- **驗證**: `/api/integrations/status` 回傳 asana 狀態

## Task 4: System Prompt
- [x] `src/lib/ai/system-prompt.ts`: 加入 Asana 使用指南區塊
- **驗證**: 選擇 Asana 資料來源時，system prompt 包含指南

## Task 5: UI Integration
- [x] `src/components/data-source-selector.tsx`: 第三方服務加入 Asana 選項
- [x] 更新 i18n：加入 asana name/description
- [x] `.env.example`: 加入 `ASANA_PAT`

...
- [x] 建立 `src/lib/integrations/github/client.ts`
...
- [x] 建立 `src/lib/integrations/github/index.ts` re-export
- [x] 建立 `src/lib/integrations/github/client.test.ts` 單元測試
- **驗證**: `npm test -- github/client` 全綠

## Task 2: GitHub AI Tool
- [x] 建立 `src/lib/ai/github-tools.ts`
...
- [x] 建立 `src/lib/ai/github-tools.test.ts` 單元測試
- **驗證**: `npm test -- github-tools` 全綠

## Task 3: Route Integration
- [x] `src/app/api/chat/route.ts`: import githubTools，加入 dataSources 過濾邏輯
- [x] `src/app/api/integrations/status/route.ts`: 回報 `github: isGitHubConfigured()`
- **驗證**: `/api/integrations/status` 回傳 github 狀態

## Task 4: System Prompt
- [x] `src/lib/ai/system-prompt.ts`: 加入 GITHUB_INSTRUCTIONS 區塊
  - 可用 actions 說明 + 參數範例
...
- [x] `src/components/data-source-selector.tsx`:「團隊協作」子選單加入 GitHub 選項（Github icon）
- [x] `src/components/tool-call-display.tsx`: 加入 githubQuery 的圖示和標籤
- [x] 更新 i18n（en.json, zh-TW.json）：github name/description 已存在，確認即可
- [x] `.env.example`: 加入 `GITHUB_TOKEN`
